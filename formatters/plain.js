...
const mapping = {
  ...
  added: ({ key, value }, propertyParents) => `Property '${[...propertyParents, key].join('.')}' was added with value: ${stringifyValue(value)}`,
  removed: ({ key }, propertyParents) => `Property '${[...propertyParents, key].join('.')}' was removed`,
  changed: ({ key, value1, value2 }, propertyParents) => `Property '${[...propertyParents, key].join('.')}' was updated. From ${stringifyValue(value1)} to ${stringifyValue(value2)}`,
  unchanged: () => [],
};
